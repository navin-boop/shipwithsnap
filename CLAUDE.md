...
| Email | `src/lib/email/` — Resend when `RESEND_API_KEY` (or `EMAIL_API_KEY`) is set, console log otherwise |
...
src/lib/email/       layout.ts (the Sunny shell in email HTML), templates.ts (every email),
                     notify.ts (event → template), index.ts (Resend transport)
...
Every email the product sends lives in `src/lib/email/`, built to the Sunny tokens inside the limits of email HTML.

- **`layout.ts`** is the shell: cream page, white card with a 2px ink outline and the offset hard shadow (drawn as an ink panel 6px larger on the right and bottom — negative margins collapse and transforms are unsupported), pill buttons, tinted callouts, label/value rows. Sora and Nunito are linked for clients that load webfonts and fall back to a heavy system stack; every colour is written literally because custom properties do not survive Gmail. Outlook flattens the radius and shadow into a plain outlined card, deliberately.
- **`templates.ts`** holds one pure function per email — data in, `{ subject, html, text }` out. No database, no network, so each can be rendered in a script and diffed in review. Every template ships a plain-text twin.
- **`notify.ts`** connects product events to templates. Account-scoped plain functions that swallow their own errors: a Resend outage must never fail a purchase that already succeeded.
- **Two audiences.** Customer mail (tracking) goes out under the seller's name and logo with ours only in the footer; account mail (receipts, invites, billing) carries the snap wordmark from `public/email/logo.png`, regenerated by `npx tsx scripts/make-brand-images.tsx`.
- **Preview them** with `npx tsx scripts/preview-emails.tsx && open email-previews/index.html`.
- Subjects are folded through `safeSubject` before sending — store names reach them from outside and a newline would forge a header. `tests/email.test.ts` renders every template with hostile input and asserts nothing escapes.

Wired: welcome, team invite, the four tracking emails, label receipt, batch receipt, refund, card declined, carrier adjustment, pickup confirmed/cancelled. Built but not yet wired: password reset (no such flow yet) and claim updates.
...
- **Postage carries a 15% markup**, defined once in `src/lib/ship/pricing.ts` and applied by `applyPricing` in `src/lib/ship/service.ts`, by the public rate calculator and by the landing-page sample — every path that turns a carrier rate into a price we show. `storeQuotes` writes the marked-up price into `rate_quotes` and `buyLabel` charges that stored row, so the price is server-authoritative and a client cannot negotiate it down. The public pages state the fee outright; `/lowest-price-guarantee` was retired (301 to `/pricing`) because it promised the opposite.
- **Insurance is the one thing we price ourselves**: EasyPost's own fee plus a 70% markup, defined once in `src/lib/ship/insurance.ts` and read by the Ship screen, the authorization, the capture and the receipt — quote and charge can never drift apart. EasyPost's fee was measured, not assumed: buying test labels at $50/$100/$500/$2,000 declared returned exactly 1% of declared value with a $1.00 floor, so the seller pays $1.70 per $100, minimum $1.70. It is on by default at a $100 declared value. It is charged on top of postage, itemised on the receipt, and refunded with the label on a void. The public pages state the price outright; postage stays pass-through. `tests/insurance.test.ts` pins the arithmetic.
